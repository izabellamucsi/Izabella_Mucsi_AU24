...
WITH insert_rental_data AS (                              --Updating the rental data for the inserted favorite films
    UPDATE public.film
    SET 
    rental_rate = CASE
...
WHERE UPPER(f.title) IN ('SENNA', 'RUSH','DRIVE TO SURVIVE')
    ...
    WHERE LOWER(c.first_name) = 'izabella' AND LOWER(last_name) = 'mucsi'
		...
		),																		-- Inserting the rental records for each movie 
			rental_insert AS (
